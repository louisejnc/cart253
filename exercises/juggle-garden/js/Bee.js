class Bee {
  ...
  constructor(x,y) {
    ...
    this.y = y;
    this.size = 40;
    this.minSize = 10; // If we get smaller, we're dead
    this.maxSize = 40;
    this.vx = 0;
    this.vy = 0;
    this.speed = 5;
    this.shrinkRate = 0.05; // How much smaller each frame
    this.growRate = 0.05;
  ...
  // then changing position based on velocity
  move() {
    // First check if we shoul change direction
    let r = random(0,1);
    ...
    // Update postion with velocity to actually move
    this.x += this.vx;
    this.y += this.vy;
